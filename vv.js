var vv = function () {
	...
	function vv(setup, cb) {
		if (setup != null) {
			...
			if (typeof setup.then === 'function')
				return vv(function (res, rej) { setup.then(res, rej); }, cb);
		...
		if (typeof cb === 'function') {
			...
			try {
				...
				if (r != null && typeof r.then === 'function')
					return vv(function (res, rej) { r.then(res, rej); }, cb);
		...
		var thunk = function thunk(cb) {
			var next, thunk = vv(function (cb) { next = cb; }, true);
			...
			try {
				if (!called) {
					...
					if (r != null && typeof r.then === 'function')
						vv(function (res, rej) { r.then(res, rej); }, callback);
				}
			} catch (err) { next(err); }
	...
	function all(arg, cb) {
		...
		return vv(arg.constructor === Array ? array : object, cb);
	...
	function nextTick(fn) {
		...
		// console.log('@@@' + cc + new Error().stack.split('\n').slice(2).filter(x=>x.indexOf('vv')>0).join('\n'));
